...
function updateCurrentProjectBible(
  ...
  const current =
    state.projectBibles[state.activeProjectId] || createDefaultBible();
  return {
    projectBibles: {
      ...state.projectBibles,
...
function resolveCanonicalImageId(
  ...
  if (
    entry?.canonicalImageId &&
    referenceImages.some((image) => image.id === entry.canonicalImageId)
  ) {
...
function getReferenceImages(
  entry?: LegacyReferenceFields | null,
): BibleReferenceImage[] {
  if (!entry) {
    return [];
  }
  ...
  return Promise.all(
    referenceImages.map(async (image) => {
      ...
      return {
        ...image,
        url:
          (await loadBlob(image.blobKey).catch(() => image.url)) ?? image.url,
      };
    }),
  );
}
...
const TRANSITION_NAMES_RE =
  /^(FADE|CUT|DISSOLVE|WIPE|IRIS|SMASH|MATCH|JUMP|FREEZE|ЗАТЕМНЕНИЕ|ПЕРЕХОД|НАПЛЫВ|ВЫТЕСНЕНИЕ|СТОП)\b/i;

function mergeCharacters(
  existing: CharacterEntry[],
  parsed: CharacterEntry[],
): CharacterEntry[] {
  ...
  for (const id of ids) {
    ...
    if (
      TRANSITION_NAMES_RE.test(name) ||
      /\bTO:?\s*$/i.test(name) ||
      /\b(IN|OUT)[.:]?\s*$/i.test(name)
    ) {
      ids.delete(id);
    }
  }
  ...
  return Array.from(ids)
    .map((id) => {
      ...
      return {
        ...
        gender: current?.gender ?? next?.gender ?? "unknown",
        ...
        generatedPortraitUrl:
          current?.generatedPortraitUrl ?? next?.generatedPortraitUrl ?? null,
        portraitBlobKey:
          current?.portraitBlobKey ?? next?.portraitBlobKey ?? null,
        appearancePrompt:
          current?.appearancePrompt ?? next?.appearancePrompt ?? "",
  ...
  persist(
    (set) => ({
      ...
      updateFromScreenplay: (blocks, scenes) => {
        const characters = linkCharactersToScenes(
          parseCharacters(blocks),
          blocks,
          scenes,
        );
      ...
      updateCharacter: (id, patch) => {
        set((state) => {
          ...
          return {
            characters,
            ...updateCurrentProjectBible(state, { characters }),
          };
        });
        emitBibleSettings();
      },
      updateLocation: (id, patch) => {
        set((state) => {
          ...
          return {
            locations,
            ...updateCurrentProjectBible(state, { locations }),
          };
        });
        emitBibleSettings();
      },
      ...
      addProp: (prop) => {
        set((state) => {
          ...
          const props = [...state.props, prop].sort((a, b) =>
      ...
      removeUnusedEntries: () => {
        ...
        useBibleStore.setState((state) => {
          const characters = state.characters.filter((c) => {
            if (c.sceneIds.length === 0 && c.dialogueCount === 0) {
              removed.chars++;
              return false;
            }
            return true;
          ...
          return {
            ...
            locations,
            props,
            ...updateCurrentProjectBible(state, {
              characters,
              locations,
              props,
            }),
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state, error) => {
        ...
        void Promise.all([
          Promise.all(
            state.characters.map(async (entry) => ({
              ...entry,
              referenceImages: await restoreReferenceImages(
                getReferenceImages(entry),
              ),
              generatedPortraitUrl: await restorePrimaryImage(
                entry.portraitBlobKey,
                entry.generatedPortraitUrl,
                entry.portraitPublicUrl,
              ),
            })),
          ),
          Promise.all(
            state.locations.map(async (entry) => ({
              ...entry,
              referenceImages: await restoreReferenceImages(
                getReferenceImages(entry),
              ),
              generatedImageUrl: await restorePrimaryImage(
                entry.imageBlobKey,
