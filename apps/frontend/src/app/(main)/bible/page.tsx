...
import {
  ...
  type VoiceConfig,
  type VoiceProvider,
  type FishAudioSettings,
  type ElevenLabsSettings,
  type WebSpeechSettings,
  type ProviderSettings,
  createDefaultSettings,
  isFishAudioSettings,
  isElevenLabsSettings,
  isWebSpeechSettings,
...
function DirectorSection() {
  ...
  const handleSelectPreset = (preset: ProductionPreset) => {
    ...
    setDirectorProfile({
      name: preset.name,
      systemPrompt: preset.directorPrompt,
    });
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div
                    className={`text-[15px] font-semibold ${isActive ? "text-[#D4A853]" : "text-white/70"}`}
                  >
                    {preset.name}
                  </div>
                  {isActive && (
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <span className="rounded-md bg-white/5 px-2 py-0.5 text-[10px] text-white/25">
                    {preset.pipeline.modules.length > 0
                      ? `${preset.pipeline.modules.length} модулей`
                      : "Дефолтный пайплайн"}
                  </span>
                </div>
              </button>
      ...
      {activePipeline && (
        <div className="rounded-xl border border-emerald-500/15 bg-emerald-500/5 p-4">
          <div className="flex items-center justify-between mb-1">
            <div className="text-[10px] uppercase tracking-[0.2em] text-emerald-400/50">
              Активный пайплайн
            </div>
          </div>
    ...
    <section className="mb-6 grid gap-4 lg:grid-cols-2">
      <article className="rounded-2xl border border-white/8 bg-white/3 p-4">
        <div className="mb-3">
          ...
          <h2 className="mt-1 text-sm font-semibold uppercase tracking-[0.14em] text-white">
            История проекта
          </h2>
        </div>
        <EditableTextArea
      ...
      <article className="rounded-2xl border border-white/8 bg-white/3 p-4 lg:col-span-2">
        <div className="mb-3">
          ...
          <h2 className="mt-1 text-sm font-semibold uppercase tracking-[0.14em] text-white">
            Стиль фона при написании
          </h2>
          <p className="mt-1 text-[11px] text-white/30">
            Промпт для генерации фоновых иллюстраций в полноэкранном режиме.
            Оставь пустым для стиля по умолчанию (comic book).
          </p>
        </div>
        <EditableTextArea
          value={ambientPrompt}
...
function buildCharacterPrompt(
  ...
  const identityInstruction =
    "Preserve the exact same facial identity, hair shape, costume silhouette, and proportions from the provided reference images.";
  const textGuard =
    "No visible text, letters, logos, monograms, or name patches in frame.";
...
function buildLocationPrompt(
  ...
  const referenceHint =
    location.referenceImages.length > 0 ? " Matching the reference style." : "";
...
function buildPropPrompt(
  ...
  const resolvedStyle = style.trim() || DEFAULT_PROJECT_STYLE;
  const referenceHint =
    prop.referenceImages.length > 0 ? " Matching the reference style." : "";
  const textGuard =
    "No visible text, letters, logos, monograms, or labels in frame.";
    ...
    const res = await apiChat("/api/chat", {
      ...
      body: JSON.stringify({
        messages: [
          {
            role: "user",
            content: `Translate to English. Return ONLY the translation, nothing else:\n\n${text}`,
          },
        ],
        temperature: 0,
      }),
  ...
  return (
    <div className="group/image relative overflow-hidden rounded-xl border border-white/8 bg-white/3">
      <div
        ...
        onDoubleClick={src ? onDoubleClick : undefined}
      >
        {src ? (
          <SmartImage
            src={src}
...
function ReferenceStrip({
  ...
  return (
    <div>
      <p className="mb-2 text-[9px] uppercase tracking-[0.18em] text-white/30">
        References
      </p>
      <div className="flex flex-wrap gap-2">
        {references.map((reference) => (
          <div
            key={reference.id}
            className={`group relative h-12 w-12 overflow-hidden rounded-md border ${canonicalImageId === reference.id ? "border-[#D4A853]" : "border-white/10"}`}
          >
            <SmartImage
              src={reference.url}
              alt="Reference"
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-white/35">{display}</span>
      </div>
      <input
        type="range"
        min={min}
...
/* ─── Fish Audio Voice Browser Modal ─── */
function FishVoiceBrowser({
  onSelect,
  onClose,
}: {
  onSelect: (voiceId: string, voiceName: string, demoUrl?: string) => void;
  onClose: () => void;
}) {
  ...
  const [sortBy, setSortBy] = useState<"score" | "task_count" | "created_at">(
    "score",
  );
  ...
  const fetchVoices = useCallback(
    async (p: number) => {
      setLoading(true);
      try {
        const { searchFishAudioVoices } = await import("@/lib/ttsProvider");
        const result = await searchFishAudioVoices({
          query: query || undefined,
          language: language || undefined,
          tag: gender || undefined,
          sortBy,
          pageSize: 12,
          pageNumber: p,
          self: showMine,
    ...
    [query, showMine, language, gender, sortBy],
  );
  ...
  const playDemo = (url: string, id: string) => {
    if (playingId === id) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="flex max-h-[80vh] w-full max-w-2xl flex-col rounded-xl border border-white/10 bg-[#1a1a2e] shadow-2xl">
        {/* Header */}
        ...
        {/* Search & Filters */}
        <div className="flex flex-col gap-2 border-b border-white/6 px-4 py-2">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-md border border-white/8 bg-white/4 px-3 py-1.5 text-[12px] text-white/80 outline-none placeholder:text-white/20"
            />
            <button
              type="button"
          ...
          <div className="flex gap-2">
            ...
            <select
              ...
              onChange={(e) =>
                setSortBy(
                  e.target.value as "score" | "task_count" | "created_at",
                )
              }
              className="rounded-md border border-white/8 bg-white/4 px-2 py-1 text-[11px] text-white/60 outline-none"
            >
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {voices.map((v) => (
                <div
                  key={v.id}
                  className="group flex flex-col gap-1.5 rounded-lg border border-white/6 bg-white/3 p-3 transition-colors hover:border-violet-500/30 hover:bg-white/5"
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {v.languages.slice(0, 3).map((l) => (
                      <span
                        key={l}
                        className="rounded bg-white/6 px-1.5 py-0.5 text-[9px] text-white/30"
                      >
                        {l}
                      </span>
                    ))}
                    {v.tags.slice(0, 2).map((t) => (
                      <span
                        key={t}
                        className="rounded bg-violet-500/10 px-1.5 py-0.5 text-[9px] text-violet-300/50"
                      >
                        {t}
                      </span>
                    ))}
                  ...
                  <div className="mt-auto flex items-center gap-1.5 pt-1">
                    {v.samples[0]?.url && (
                      <button
                        type="button"
                        onClick={() => playDemo(v.samples[0].url, v.id)}
                        className="flex items-center gap-1 rounded bg-white/5 px-2 py-1 text-[10px] text-white/40 transition-colors hover:text-white/70"
                      >
                        {playingId === v.id ? (
                    ...
                    <button
                      type="button"
                      onClick={() => onSelect(v.id, v.name, v.samples[0]?.url)}
                      className="ml-auto rounded bg-violet-500/15 px-2 py-1 text-[10px] text-violet-300 transition-colors hover:bg-violet-500/25"
                    >
                      Select
                    </button>
                  </div>
        ...
        {/* Pagination */}
        {total > 12 && (
          <div className="flex items-center justify-center gap-3 border-t border-white/6 px-4 py-2">
            <button
              type="button"
              disabled={page <= 1}
              onClick={() => {
              ...
              className="rounded px-2 py-1 text-[10px] text-white/40 hover:text-white/60 disabled:opacity-30"
            >
              Prev
            </button>
...
/* ─── Voice Section (provider-specific controls) ─── */
...
function VoiceSection({
  ...
  // Restore preview blob from IndexedDB on mount
  useEffect(() => {
    if (!characterName) return;
    const blobKey = `bible-voice-preview-${characterName.toLowerCase().replace(/\s+/g, "-")}`;
    let cancelled = false;
    import("@/lib/fileStorage")
      .then(({ loadBlob }) =>
        loadBlob(blobKey).then((url) => {
          if (url && !cancelled) previewBlobUrlRef.current = url;
        }),
      )
      .catch(() => {});
    return () => {
  ...
  const handlePlay = () => {
    const url = voice?.previewUrl || previewBlobUrlRef.current;
    if (!url) return;
  ...
  const handlePreview = async () => {
    ...
    setPreviewError("");
    try {
      const isRussian = /[а-яёА-ЯЁ]/.test(previewText || characterName);
      ...
      const providerType =
        current.provider === "web-speech" ||
        current.provider === "elevenlabs" ||
        current.provider === "fish-audio"
          ? current.provider
          : ("web-speech" as const);

      // For web-speech, use SpeechSynthesis directly (avoids MediaRecorder issues)
      if (
        ...
        utterance.lang = lang;
        const ws = current.settings as WebSpeechSettings | undefined;
        utterance.rate = ws?.speed ?? 1.0;
        utterance.pitch = ws?.pitch ?? 1.0;
        if (current.voiceId) {
          const v = window.speechSynthesis
            .getVoices()
            .find((v) => v.voiceURI === current.voiceId);
      ...
      const { generateSpeech } = await import("@/lib/ttsProvider");
      const result = await generateSpeech(
        {
          text: sampleText,
          lang,
          voiceId: current.voiceId || undefined,
      ...
      if (result.audioBlob.size > 0) {
        ...
        // Persist preview blob to IndexedDB for cross-session survival
        if (characterName) {
          const blobKey = `bible-voice-preview-${characterName.toLowerCase().replace(/\s+/g, "-")}`;
  ...
  const updateSettings = (patch: Partial<ProviderSettings>) => {
    ...
    onUpdate({
      ...voice,
      settings: { ...voice.settings, ...patch } as ProviderSettings,
    });
  };
      ...
      <button
        ...
        onClick={() => {
          onUpdate({
            ...
            voiceId: "",
            voiceName: "",
            settings: createDefaultSettings("web-speech"),
          });
          setExpanded(true);
        }}
  ...
  const current = voice ?? {
    provider: "web-speech" as VoiceProvider,
    voiceId: "",
    voiceName: "",
    settings: createDefaultSettings("web-speech"),
  };

  const fishSettings = isFishAudioSettings(current.settings)
    ? current.settings
    : null;
  const elevenSettings = isElevenLabsSettings(current.settings)
    ? current.settings
    : null;
  const wsSettings = isWebSpeechSettings(current.settings)
    ? current.settings
    : null;
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        <ChevronDown
          className={`h-3 w-3 text-white/25 transition-transform ${expanded ? "rotate-180" : ""}`}
        />
      </button>
      ...
      {expanded && (
        <div className="space-y-2 rounded-lg border border-white/6 bg-white/2 p-3">
          {/* Provider */}
          <div>
            <p className="mb-1 text-[9px] uppercase tracking-[0.16em] text-white/25">
              Provider
            </p>
                ...
                <button
                  ...
                  onClick={() =>
                    onUpdate({
                      ...current,
                      provider: p.id,
                      settings: createDefaultSettings(p.id),
                    })
                  }
              ...
              {webVoices.length > 0 && (
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-md border border-white/8 bg-white/4 px-2 py-1.5 text-[11px] text-white/80 outline-none"
                  >
          ...
          {current.provider === "fish-audio" && (
            <>
              ...
              <div>
                <div className="mb-1 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[9px] text-violet-400/60 transition-colors hover:text-violet-400"
                  >
                    Browse catalog
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    value={current.voiceId}
                    onChange={(e) =>
                      onUpdate({ ...current, voiceId: e.target.value })
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      onUpdate({ ...current, voiceName: e.target.value })
                    }
              ...
              <SettingSlider
                ...
                value={fishSettings?.temperature ?? 0.7}
                min={0}
                max={1}
                step={0.05}
              ...
              <div className="relative">
                ...
                <div className="relative">
                  <input
                    type="text"
                    value={fishSettings?.emotion ?? ""}
                    onChange={(e) =>
                      updateSettings({ emotion: e.target.value || undefined })
                    }
                    onFocus={() => setEmotionOpen(true)}
                    placeholder="whisper, angry, calm..."
                    className="w-full rounded-md border border-white/8 bg-white/4 px-2 py-1.5 text-[11px] text-white/80 outline-none placeholder:text-white/15"
                  />
                  {emotionOpen && (
                    <div className="absolute left-0 top-full z-30 mt-1 max-h-36 w-full overflow-y-auto rounded-md border border-white/10 bg-[#1a1a2e] shadow-lg">
                      {FISH_EMOTION_PRESETS.map((em) => (
                        <button
                          key={em}
                          type="button"
                          onMouseDown={(e) => e.preventDefault()}
                          onClick={() => {
                            updateSettings({ emotion: em });
                            setEmotionOpen(false);
                          }}
                          className="flex w-full items-center px-2.5 py-1.5 text-left text-[11px] text-white/60 transition-colors hover:bg-white/6 hover:text-white/90"
                        >
                          [{em}]
                        </button>
                      ))}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <select
                    value={fishSettings?.format ?? "mp3"}
                    onChange={(e) =>
                      updateSettings({
                        format: e.target.value as "mp3" | "wav" | "opus",
                      })
                    }
                    className="w-full rounded-md border border-white/8 bg-white/4 px-2 py-1.5 text-[11px] text-white/80 outline-none"
                  >
                ...
                <div>
                  ...
                  <select
                    value={fishSettings?.latency ?? "normal"}
                    onChange={(e) =>
                      updateSettings({
                        latency: e.target.value as
                          | "low"
                          | "normal"
                          | "balanced",
                      })
          ...
          {current.provider === "elevenlabs" && (
            <>
              ...
              <SettingSlider
                label="Stability"
                value={elevenSettings?.stability ?? 0.5}
                min={0}
                max={1}
                step={0.05}
                onChange={(v) => updateSettings({ stability: v })}
              />
              <SettingSlider
                label="Similarity Boost"
                value={elevenSettings?.similarityBoost ?? 0.75}
                min={0}
                max={1}
                step={0.05}
                onChange={(v) => updateSettings({ similarityBoost: v })}
              />
              <SettingSlider
                label="Style"
                value={elevenSettings?.style ?? 0.5}
                min={0}
                max={1}
                step={0.05}
              ...
              <SettingSlider
                label="Speed"
                value={elevenSettings?.speed ?? 1.0}
                min={0.5}
                max={2.0}
                step={0.1}
                format={(v) => `${v.toFixed(1)}x`}
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() =>
                    updateSettings({
                      useSpeakerBoost: !(
                        elevenSettings?.useSpeakerBoost ?? true
                      ),
                    })
                  }
                ...
                <span className="text-[10px] text-white/40">Speaker Boost</span>
              </div>
            </>
          )}
          ...
          <div className="flex flex-col gap-2">
            <textarea
              ...
              placeholder={`Hello, my name is ${characterName}...`}
              rows={2}
              className="w-full resize-none rounded-md border border-white/8 bg-white/4 px-2.5 py-1.5 text-[11px] text-white/80 outline-none placeholder:text-white/20"
            />
            <div className="flex items-center gap-2">
              {/* Demo — plays the original template sample (instant, free) */}
              {(current.previewUrl || previewBlobUrlRef.current) && (
                <button
                  type="button"
                  onClick={() => {
                    if (playing && audioRef.current) {
                    ...
                    const url = current.previewUrl || previewBlobUrlRef.current;
                    if (!url) return;
                    const audio = new Audio(url);
                    audioRef.current = audio;
                    audio.onended = () => setPlaying(false);
                  ...
                  className="flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-3 py-1.5 text-[10px] text-white/60 transition-colors hover:bg-white/10 hover:text-white/80"
                >
                  {playing && !previewing ? (
                  ...
                  {playing && !previewing ? "Stop" : "Demo"}
                </button>
              )}
              {/* Preview — generates new audio with current settings (uses API) */}
              <button
                type="button"
                onClick={() => {
                  if (playing && audioRef.current) {
                ...
                disabled={previewing}
                className="flex items-center gap-1.5 rounded-md border border-violet-500/20 bg-violet-500/10 px-3 py-1.5 text-[10px] text-violet-300 transition-colors hover:bg-violet-500/20 disabled:opacity-50"
              >
                {previewing ? (
                ...
                {previewing ? "Generating..." : "Preview settings"}
              </button>
            </div>
            {previewError && (
              <p className="text-[10px] text-red-400/70">{previewError}</p>
            )}
          </div>
      ...
      {showVoiceBrowser && (
        <FishVoiceBrowser
          onSelect={(voiceId, voiceName, demoUrl) => {
            onUpdate({
              ...current,
              voiceId,
              voiceName,
              previewUrl: demoUrl || current.previewUrl,
            });
  ...
  return (
    <article className="w-full max-w-70 rounded-xl border border-white/8 bg-white/2 p-3 transition-all duration-200 hover:bg-white/4">
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        ...
        >
          {entry.canonicalImageId === GENERATED_CANONICAL_IMAGE_ID
            ? "Canonical"
            : "Use as anchor"}
        </button>
      </div>
      ...
      <div className="mt-3 space-y-3">
        <div>
          <h2 className="text-[14px] font-bold uppercase tracking-[0.14em] text-white">
            {entry.name}
          </h2>
          <p className="mt-1 text-[10px] text-white/40">
            {formatLineCount(entry.dialogueCount)}
          </p>
            ...
            <div className="flex gap-1">
              {(["male", "female", "unknown"] as const).map((g) => (
                <button
                  ...
                  className={`rounded-md px-2 py-0.5 text-[9px] uppercase tracking-wide transition-colors ${
                    entry.gender === g
                      ? "bg-violet-500/20 text-violet-300"
                      : "bg-white/4 text-white/30 hover:bg-white/8"
                  }`}
                >
                  {g === "male" ? "M" : g === "female" ? "F" : "?"}
                </button>
...
- description — на РУССКОМ`,
  ...
  if (!res.ok)
    return { characters: [], locations: [], props: [], newProps: [] };
    ...
    <article className="rounded-xl border border-dashed border-[#D4A853]/30 bg-[#D4A853]/4 p-3 transition-all duration-200">
      <div className="mb-2 flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[13px] font-semibold uppercase tracking-widest text-[#E8D7B2]">
              {suggestion.name}
            </h3>
            <p className="text-[10px] text-white/30">
              {suggestion.sourceScenes.length}{" "}
              {suggestion.sourceScenes.length === 1 ? "сцена" : "сцен"}
            </p>
          </div>
        </div>
        <div className="flex gap-1">
      ...
      <p className="mt-1 text-[10px] italic text-[#D4A853]/40">
        {suggestion.reason}
      </p>
      {suggestion.appearancePrompt ? (
        <p className="mt-1 text-[10px] text-white/30">
          Prompt: {suggestion.appearancePrompt}
        </p>
      ) : null}
    </article>
  );
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {sorted.map((n) => (
        <span
          key={n}
          className="rounded bg-white/6 px-1.5 py-0.5 text-[9px] tabular-nums text-white/35"
        >
          S{n}
        </span>
      ))}
...
export default function BiblePage() {
  ...
  // Edit overlay state
  const [editOverlayTarget, setEditOverlayTarget] = useState<{
    id: string;
    type: "char" | "loc" | "prop";
    src: string;
  } | null>(null);
  ...
  const [lbTransform, setLbTransform] = useState({
    flipH: false,
    flipV: false,
    rotate: 0,
  });
  ...
  const updateFromScreenplay = useBibleStore(
    (state) => state.updateFromScreenplay,
  );
  ...
  const removeUnusedEntries = useBibleStore(
    (state) => state.removeUnusedEntries,
  );
  const updateStoryHistory = useBibleStore((state) => state.updateStoryHistory);
  const updateDirectorVision = useBibleStore(
    (state) => state.updateDirectorVision,
  );
  const ambientPrompt = useBibleStore((state) => state.ambientPrompt);
  const updateAmbientPrompt = useBibleStore(
    (state) => state.updateAmbientPrompt,
  );
  const selectedImageGenModel = useBoardStore(
    (state) => state.selectedImageGenModel,
  );
  const setSelectedImageGenModel = useBoardStore(
    (state) => state.setSelectedImageGenModel,
  );
  ...
  const firstSceneIndex = useCallback(
    (sceneIds: string[]) => {
      let min = Infinity;
  ...
  const sortedCharacters = useMemo(
    () =>
      [...characters].sort(
        (a, b) => firstSceneIndex(a.sceneIds) - firstSceneIndex(b.sceneIds),
      ),
  ...
  const handleCharacterGenerate = async (character: CharacterEntry) => {
    ...
    try {
      ...
      const translated =
        model === "gpt-image" && character.appearancePrompt.trim()
      ...
      const userRefs = getCharacterGenerationReferenceImages(character).filter(
        (ref) => ref.id !== "__generated_primary__",
      );
      ...
      const result = await replaceBlobImage(
        character.generatedPortraitUrl,
        character.portraitBlobKey,
        blobKey,
        resBlob,
      );
      ...
      updateCharacter(character.id, {
        canonicalImageId:
          character.canonicalImageId ?? GENERATED_CANONICAL_IMAGE_ID,
  ...
  const handleLocationGenerate = async (location: LocationEntry) => {
    ...
    try {
      ...
      const userRefs = getLocationGenerationReferenceImages(location).filter(
        (ref) => ref.id !== "__generated_primary__",
      );
      const referenceImages = await convertReferenceImagesToDataUrls(userRefs);
      ...
      const result = await replaceBlobImage(
        location.generatedImageUrl,
        location.imageBlobKey,
        blobKey,
        blob,
      );
      ...
      updateLocation(location.id, {
        canonicalImageId:
          location.canonicalImageId ?? GENERATED_CANONICAL_IMAGE_ID,
  ...
  const handleCharacterPrimaryUpload = async (
    character: CharacterEntry,
    file: File,
  ) => {
    const blobKey = `bible-char-${character.id}-${Date.now()}`;
  ...
  const handleLocationPrimaryUpload = async (
    location: LocationEntry,
    file: File,
  ) => {
    const blobKey = `bible-location-${location.id}-${Date.now()}`;
  ...
  const handleCharacterReferenceAdd = async (
    character: CharacterEntry,
    files: File[],
  ) => {
    ...
    updateCharacter(character.id, {
      canonicalImageId:
        ...
        (character.generatedPortraitUrl
          ? GENERATED_CANONICAL_IMAGE_ID
          : (newReferences[0]?.id ?? null)),
      referenceImages: [...character.referenceImages, ...newReferences],
    });
  };

  const handleLocationReferenceAdd = async (
    location: LocationEntry,
    files: File[],
  ) => {
  ...
  const handleCharacterReferenceRemove = async (
    character: CharacterEntry,
    reference: BibleReferenceImage,
  ) => {
    await deleteBlob(reference.blobKey).catch(() => undefined);
    URL.revokeObjectURL(reference.url);
    const nextReferences = character.referenceImages.filter(
      (item) => item.id !== reference.id,
    );
    updateCharacter(character.id, {
      canonicalImageId:
        character.canonicalImageId === reference.id
          ? character.generatedPortraitUrl
            ? GENERATED_CANONICAL_IMAGE_ID
  ...
  const handleLocationReferenceRemove = async (
    location: LocationEntry,
    reference: BibleReferenceImage,
  ) => {
    await deleteBlob(reference.blobKey).catch(() => undefined);
    URL.revokeObjectURL(reference.url);
    const nextReferences = location.referenceImages.filter(
      (item) => item.id !== reference.id,
    );
    updateLocation(location.id, {
      canonicalImageId:
        location.canonicalImageId === reference.id
          ? location.generatedImageUrl
            ? GENERATED_CANONICAL_IMAGE_ID
            : (nextReferences[0]?.id ?? null)
          : location.canonicalImageId,
  ...
  const handlePropGenerate = async (prop: PropEntry) => {
    ...
    try {
      ...
      const translated = needsTranslation
        ? await translateToEnglish(prop.appearancePrompt || prop.name)
        : undefined;
      const prompt = buildPropPrompt(prop, projectStyle, translated);
      const userRefs = getPropGenerationReferenceImages(prop).filter(
        (ref) => ref.id !== "__generated_primary__",
      );
      const referenceImages = await convertReferenceImagesToDataUrls(userRefs);
      const blob = await generateBibleImage(
        prompt,
  ...
  const handlePropReferenceRemove = async (
    prop: PropEntry,
    reference: BibleReferenceImage,
  ) => {
    await deleteBlob(reference.blobKey).catch(() => undefined);
    URL.revokeObjectURL(reference.url);
    const nextReferences = prop.referenceImages.filter(
      (item) => item.id !== reference.id,
    );
    updateProp(prop.id, {
      canonicalImageId:
        prop.canonicalImageId === reference.id
          ? prop.generatedImageUrl
            ? GENERATED_CANONICAL_IMAGE_ID
            : (nextReferences[0]?.id ?? null)
          : prop.canonicalImageId,
  ...
  const unusedCount = useMemo(() => {
    const chars = characters.filter(
      (c) => c.sceneIds.length === 0 && c.dialogueCount === 0,
    ).length;
  ...
  const handleSmartScan = async () => {
    ...
    try {
      ...
      setScanStats({
        chars: charsFilled,
        locs: locsFilled,
        props: propsFilled + result.newProps.length,
      });
    } catch (err) {
  ...
  const bakeTransform = async () => {
    ...
    if (lightbox.type === "char")
      updateCharacter(lightbox.id, {
        generatedPortraitUrl: result.url,
        portraitBlobKey: result.s3Key ? null : blobKey,
        portraitS3Key: result.s3Key,
        portraitPublicUrl: result.publicUrl,
      });
    else if (lightbox.type === "loc")
      updateLocation(lightbox.id, {
        generatedImageUrl: result.url,
        imageBlobKey: result.s3Key ? null : blobKey,
        imageS3Key: result.s3Key,
        imagePublicUrl: result.publicUrl,
  ...
  const hasLbTransform =
    lbTransform.flipH || lbTransform.flipV || lbTransform.rotate !== 0;
    ...
    <main className="min-h-screen bg-[#0E0D0B] text-white">
        ...
        <header className="sticky top-0 z-20 mb-6 border-b border-white/8 bg-[#0E0D0B]/92 pb-5 backdrop-blur">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 text-[#D4A853]">
              ...
              <span className="text-sm uppercase tracking-[0.32em] text-white/65">
                PIECE BIBLE
              </span>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <div className="flex items-center gap-1 rounded-md border border-white/10 bg-white/5 px-1">
                <span className="px-1 text-[8px] uppercase tracking-wider text-white/30">
                  Model:
                </span>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          {scanning ? (
            ...
            <span className="text-[11px] text-white/40">
              Заполнено:{" "}
              {scanStats.chars > 0 ? `${scanStats.chars} персонажей` : ""}
        ...
        {suggestions.length > 0 ? (
          <div className="mb-6">
            <p className="mb-3 text-[10px] uppercase tracking-[0.18em] text-[#D4A853]/60">
              ...
              {suggestions.length === 1 ? "предмет" : "предметов"}
            </p>
            ...
            <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
              {sortedProps.map((entry) => (
                <PropCard
                  ...
                  onSetCanonicalPrimary={() =>
                    updateProp(entry.id, {
                      canonicalImageId: GENERATED_CANONICAL_IMAGE_ID,
                    })
                  }
                  onUploadPrimary={(file) =>
                    void handlePropPrimaryUpload(entry, file)
                  }
                  onAddReferences={(files) =>
                    void handlePropReferenceAdd(entry, files)
                  }
                  onSetCanonicalReference={(reference) =>
                    updateProp(entry.id, { canonicalImageId: reference.id })
                  }
                  onRemoveReference={(reference) =>
                    void handlePropReferenceRemove(entry, reference)
                  }
                  onEdit={() =>
                    entry.generatedImageUrl &&
                    openEditOverlay(entry.id, "prop", entry.generatedImageUrl)
                  }
                  onDoubleClick={() =>
                    entry.generatedImageUrl &&
                    openLightbox(entry.id, "prop", entry.generatedImageUrl)
                  }
          ...
          <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
            {sortedCharacters.map((entry) => (
              <CharacterCard
                ...
                onUploadPrimary={(file) =>
                  void handleCharacterPrimaryUpload(entry, file)
                }
                onAddReferences={(files) =>
                  void handleCharacterReferenceAdd(entry, files)
                }
                onSetCanonicalReference={(reference) =>
                  updateCharacter(entry.id, { canonicalImageId: reference.id })
                }
                onRemoveReference={(reference) =>
                  void handleCharacterReferenceRemove(entry, reference)
                }
                onEdit={() =>
                  entry.generatedPortraitUrl &&
                  openEditOverlay(entry.id, "char", entry.generatedPortraitUrl)
                }
                onDoubleClick={() =>
                  entry.generatedPortraitUrl &&
                  openLightbox(entry.id, "char", entry.generatedPortraitUrl)
                }
                editingThis={editingId === entry.id}
              />
            ))}
            {characters.length === 0 ? (
              <div className="col-span-full rounded-xl border border-dashed border-white/10 bg-white/2 p-10 text-center text-white/45">
                No characters parsed yet. Add character blocks to the
                screenplay.
              </div>
            ) : null}
          </section>
          ...
          <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
            {sortedLocations.map((entry) => (
              <LocationCard
                ...
                onUploadPrimary={(file) =>
                  void handleLocationPrimaryUpload(entry, file)
                }
                onAddReferences={(files) =>
                  void handleLocationReferenceAdd(entry, files)
                }
                onSetCanonicalReference={(reference) =>
                  updateLocation(entry.id, { canonicalImageId: reference.id })
                }
                onRemoveReference={(reference) =>
                  void handleLocationReferenceRemove(entry, reference)
                }
                onEdit={() =>
                  entry.generatedImageUrl &&
                  openEditOverlay(entry.id, "loc", entry.generatedImageUrl)
                }
                onDoubleClick={() =>
                  entry.generatedImageUrl &&
                  openLightbox(entry.id, "loc", entry.generatedImageUrl)
                }
                editingThis={editingId === entry.id}
              />
            ))}
      ...
      {lightbox && (
        <div
          ...
          onClick={() => {
            if (hasLbTransform) void bakeTransform();
            setLightbox(null);
          }}
        >
          <div
            className="relative flex max-h-[80vh] max-w-[75vw] flex-col items-center gap-4"
          ...
          >
            ...
            <div className="flex items-center gap-1 rounded-xl border border-white/10 bg-[#1A1916]/95 px-2 py-1.5 shadow-2xl backdrop-blur-xl">
              <button
                type="button"
                onClick={handleLbCopy}
                className="flex h-7 w-7 items-center justify-center rounded-lg text-white/50 transition-colors hover:bg-white/8 hover:text-white"
                title="Copy"
              >
                <Copy size={14} />
              </button>
              ...
              <button
                type="button"
                onClick={() =>
                  openEditOverlay(lightbox.id, lightbox.type, lightbox.src)
                }
                className="flex h-7 items-center gap-1 rounded-lg px-2 text-[10px] uppercase tracking-widest text-[#D4A853] transition-colors hover:bg-[#D4A853]/10"
                title="Smart Edit"
              >
                <Pencil size={12} />
                Edit
              ...
              <button
                type="button"
                onClick={() =>
                  setLbTransform((t) => ({ ...t, flipH: !t.flipH }))
                }
                className="flex h-7 w-7 items-center justify-center rounded-lg text-white/50 transition-colors hover:bg-white/8 hover:text-white"
                title="Flip H"
              >
              ...
              <button
                type="button"
                onClick={() =>
                  setLbTransform((t) => ({ ...t, flipV: !t.flipV }))
                }
                className="flex h-7 w-7 items-center justify-center rounded-lg text-white/50 transition-colors hover:bg-white/8 hover:text-white"
                title="Flip V"
              >
              ...
              {hasLbTransform && (
                <>
                  ...
                  <button
                    ...
                    className="flex h-7 items-center gap-1 rounded-lg bg-[#D4A853]/10 px-2 text-[10px] uppercase tracking-widest text-[#D4A853] transition-colors hover:bg-[#D4A853]/20"
                  >
                    <Check size={12} />
                    Apply
                  </button>
