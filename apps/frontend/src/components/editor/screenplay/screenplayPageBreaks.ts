/**
 * Piece Screenplay Page Break Engine
 *
...
const CONTENT_WIDTH =
  ...
  SCREENPLAY_PAGE_PADDING_LEFT_PX -
  SCREENPLAY_PAGE_PADDING_RIGHT_PX;
const CHARS_PER_LINE = Math.max(
  20,
  Math.floor(CONTENT_WIDTH / (SCREENPLAY_FONT_SIZE_PX * 0.6)),
);
...
function getMarginTop(
  ...
  if (type === "action") {
    if (prevType === "scene_heading")
      return SCREENPLAY_ACTION_AFTER_SCENE_HEADING_MARGIN_TOP_PX;
    if (prevType === "action")
      return SCREENPLAY_ACTION_AFTER_ACTION_MARGIN_TOP_PX;
  }
  return 0;
}
...
function getAvailChars(type: ScreenplayElementType): number {
  ...
  if (type === "character") avail -= SCREENPLAY_CHARACTER_INDENT_CH;
    ...
    avail -=
      SCREENPLAY_DIALOGUE_INDENT_LEFT_CH + SCREENPLAY_DIALOGUE_INDENT_RIGHT_CH;
  else if (type === "parenthetical")
    avail -= SCREENPLAY_PARENTHETICAL_INDENT_CH;
...
function pageContentStart(pageIdx: number): number {
  return (
    pageIdx * (SCREENPLAY_PAGE_HEIGHT_PX + SCREENPLAY_PAGE_GAP_PX) +
    SCREENPLAY_PAGE_PADDING_TOP_PX
  );
}
  ...
  for (let i = 0; i < elems.length; i++) {
    ...
    if (elem.type === "transition" && elemBottom <= curPageEnd) {
      ...
      if (nextElem && nextElem.type === "scene_heading") {
        const nextBottom =
          elemBottom +
          getMarginTop(nextElem.type, elem.type) +
          nextElem.lines * LINE_H;
        if (nextBottom > curPageEnd) {
          pushToNextPage(i);
          continue;
        }
    ...
    if (
      (elem.type === "dialogue" || elem.type === "action") &&
      elem.lines > 1
    ) {
      if (
        linesOnThisPage >= MIN_LINES_ON_SIDE &&
        elem.lines - linesOnThisPage >= MIN_LINES_ON_SIDE
      ) {
