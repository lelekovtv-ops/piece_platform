...
export const DIRECTOR_FIELD_VISIBILITY_OPTIONS: Array<{
...
}> = [
  {
    value: "all",
    label: "Show All",
    description: "Action, Director, and Camera",
  },
  {
    value: "action",
    label: "Action Only",
    description: "Keep only the action line",
  },
  {
    value: "director",
    label: "Director Only",
    description: "Keep only director notes",
  },
  {
    value: "camera",
    label: "Camera Only",
    description: "Keep only camera notes",
  },
];
...
export function buildSceneContextText(
  sceneId: string | null,
  scenes: ReturnType<typeof useScenesStore.getState>["scenes"],
  scriptBlocks: ReturnType<typeof useScriptStore.getState>["blocks"],
): string {
  if (!sceneId) return "";
  ...
  const excerpt = scene.blockIds
    .slice(0, 6)
    .map((blockId) =>
      scriptBlocks.find((block) => block.id === blockId)?.text?.trim(),
    )
    .filter(Boolean)
    .join(" ")
    .slice(0, 500);

  return [`Scene: ${scene.title}`, excerpt ? `Excerpt: ${excerpt}` : ""]
    .filter(Boolean)
    .join("\n");
}
...
export function applyLucBessonProfileToScene({
  ...
  lucBessonProfile: NonNullable<
    ReturnType<
      typeof useProjectProfilesStore.getState
    >["lucBessonByProjectId"][string]
  >;
  ...
  useBibleStore.setState((state) => {
    ...
    lucBessonProfile.characterOverrides.forEach((override) => {
      ...
      if (existingIndex >= 0) {
        ...
        nextCharacters[existingIndex] = {
          ...current,
          description: mergeProfileText(
            current.description,
            override.description,
          ),
          appearancePrompt: mergeProfileText(
            current.appearancePrompt,
            override.appearancePrompt,
          ),
          sceneIds: current.sceneIds.includes(targetScene.id)
            ? current.sceneIds
            : [...current.sceneIds, targetScene.id],
        };
        return;
      }
    ...
    lucBessonProfile.locationOverrides.forEach((override) => {
      const entryId = slugify(override.name);
      const existingIndex = nextLocations.findIndex(
        (entry) => entry.id === entryId,
      );
  ...
  replaceSceneShots(
    targetScene.id,
    lucBessonProfile.sceneText.trim() ||
      selectedSceneContext ||
      targetScene.title,
