...
import {
  buildImagePrompt,
  getReferencedBibleEntries,
} from "@/lib/promptBuilder";
import {
  getShotGenerationReferenceImages,
...
export function DirectorFieldVisibilityControl({
  ...
  const activeOption =
    DIRECTOR_FIELD_VISIBILITY_OPTIONS.find(
      (option) => option.value === value,
    ) ?? DIRECTOR_FIELD_VISIBILITY_OPTIONS[0];
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        <div className="leading-none">
          <p className="text-[9px] uppercase tracking-[0.18em] text-[#8D919B]">
            Fields
          </p>
          <p className="mt-1 text-[11px] text-[#E7E3DC]">
            {activeOption.label}
          </p>
        </div>
      </button>
        ...
        <div className="absolute right-0 top-[calc(100%+8px)] z-20 w-64 overflow-hidden rounded-2xl border border-white/8 bg-[#171A20]/96 p-1.5 shadow-[0_24px_60px_rgba(0,0,0,0.35)] backdrop-blur-xl">
          {DIRECTOR_FIELD_VISIBILITY_OPTIONS.map((option) => {
            ...
            return (
              ...
              >
                <span
                  className={`mt-0.5 h-2.5 w-2.5 shrink-0 rounded-full ${isActive ? "bg-white/60" : "bg-white/12"}`}
                />
                <span className="min-w-0">
                  <span className="block text-[11px] uppercase tracking-[0.16em]">
                    {option.label}
                  </span>
                  <span className="mt-1 block text-[11px] normal-case tracking-normal text-[#8D919B]">
                    {option.description}
                  </span>
                </span>
              </button>
...
export function DirectorShotCard({
  ...
  const showDirector =
    fieldVisibility === "all" || fieldVisibility === "director";
    ...
    >
      {previewSrc ? (
        ...
        <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.05),transparent_58%),linear-gradient(180deg,#151922_0%,#0E1014_100%)] px-3 text-center">
          <div>
            <span className="inline-flex items-center justify-center rounded-md bg-white/8 px-2.5 py-1 text-[18px] font-bold tabular-nums text-white/50">
              {slateNumber ?? String(index + 1)}
            </span>
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex items-center justify-center rounded-md px-2 py-0.5 text-[14px] font-bold tabular-nums tracking-[0.06em] ${selected ? "bg-[#D4A853]/20 text-[#D4A853]" : "bg-white/8 text-white/70"}`}
          >
            {slateNumber ?? String(index + 1)}
          </span>
          <input
            ...
            onBlur={(e) => {
              ...
              if (!isNaN(val) && val > 0)
                onUpdate({ duration: Math.round(val * 1000) });
            }}
      ...
      {isSingleFieldMode ? (
        <div
          className={`mt-3 flex h-40 items-stretch ${showThumbnail ? "gap-4" : "gap-0"}`}
        >
          <div className="min-w-0 flex-1 rounded-2xl border border-white/8 bg-white/3 px-3 py-3">
            <div className="mb-2 flex items-center justify-between gap-3">
              <span className="text-[11px] uppercase tracking-[0.18em] text-[#8D919B]">
                {singleFieldConfig.label}
              </span>
            </div>
            <DebouncedTextarea
        ...
        <div
          className={`mt-2.5 flex items-start ${showThumbnail ? "gap-4" : "gap-0"}`}
        >
          <div className="min-w-0 flex-1">
            <div className="flex flex-col gap-1.5">
              {!isSingleFieldMode && showAction ? (
                <div
                  className={`flex gap-3 rounded-xl border px-3 transition-[border-color,background-color] duration-100 ease-out ${selected ? "border-[#DCC7A3]/22 bg-white/5" : "border-white/8 bg-white/3"} ${isSplitCompactFields ? "items-start py-2" : "items-center py-1.5 min-h-10"}`}
                >
                  <span
                    className={`w-20 shrink-0 text-[11px] uppercase tracking-[0.18em] text-[#8D919B] ${isSplitCompactFields ? "pt-1" : ""}`}
                  >
                    Action
                  </span>
                  <DebouncedTextarea
                          ...
                          <div key={entry.id} className="group/pill relative">
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                onUpdate({
                                  excludedBibleIds: Array.from(next),
                                });
                              }}
                            ...
                            <div className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 rounded-lg border border-white/10 bg-black/90 p-1.5 opacity-0 shadow-xl backdrop-blur-xl transition-opacity group-hover/pill:opacity-100">
                              {entry.imageUrl ? (
                                ...
                                <div className="flex h-16 w-16 items-center justify-center rounded-md bg-white/5 text-[8px] text-white/30">
                                  No image
                                </div>
                              )}
                              <p className="mt-1 max-w-[120px] truncate text-center text-[8px] text-white/50">
                                {entry.prompt || entry.label}
                              </p>
                            </div>
                          </div>
                        );
          ...
          {promptExpanded &&
            (() => {
              const fullPrompt = buildImagePrompt(
                ...
                bibleChars,
                bibleLocs,
                useBoardStore.getState().projectStyle,
                bibleProps,
              );
...
export async function generateShotImage(
  ...
  const references = getShotGenerationReferenceImages(
    shot,
    characters,
    locations,
  );
  const referenceImages = await convertReferenceImagesToDataUrls(references);
  const { characters: mentionedChars, location } = getReferencedBibleEntries(
    shot,
    characters,
    locations,
    bibleProps,
  ...
  const charRefs = mentionedChars
    .map(
      (character) =>
        `${character.name}: ${character.appearancePrompt || character.description || "no description"}`,
    )
    .join("\n");
  const locRef =
    location?.appearancePrompt || location?.description || location?.name || "";
  ...
  try {
    ...
    console.log("[PIECE] Image response received via generateContent");
    const blob = result.blob;
    console.log(`[PIECE] Blob created: ${blob.size} bytes, type: ${blob.type}`);
    if (blob.size < 1000) {
      throw new Error(
        `Image too small (${blob.size} bytes), likely an error response`,
      );
    }
    ...
    const objectUrl = adaptive.remote
      ? adaptive.url
    ...
    if (!adaptive.remote && !adaptive.url) {
      devlog.warn(
        ...
        "The image was generated, but local blob persistence failed. The preview will work until reload.",
        {
          shotId: shot.id,
          model: selectedModel,
        },
    ...
    console.log(
      `[PIECE] Image generated in ${Date.now() - start}ms via ${selectedModel}`,
    );
