...
type ServerMessage =
  ...
  | {
      type: "snapshot";
      data: Awaited<ReturnType<typeof getProjectSnapshot>>;
      locks: Awaited<ReturnType<typeof getProjectLocks>>;
...
const presence = new Map<
  string,
  { name: string; cursor: PresenceInfo["cursor"] }
>(); // userId → presence
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", async (raw) => {
    ...
    if (msg.type === "join") {
      const hasAccess =
        (await canWrite(state.user.userId, msg.projectId)) ||
        (await import("./auth.js")).getUserProjectRole(
          state.user.userId,
          msg.projectId,
        ) !== null;
    ...
    if (msg.type === "leave") {
      if (state.projectId) {
        ...
        broadcastToProject(
          state.projectId,
          { type: "user:left", userId: state.user.userId },
          ws,
        );
...
httpServer.listen(PORT, () => {
  console.log(`[piece-ws] WebSocket server running on port ${PORT}`);
});
