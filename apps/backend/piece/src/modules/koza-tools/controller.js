...
import { createTask as createSjinnTask, pollTask as pollSjinnTask } from './services/sjinn.js';
import { createTask as createTripoTask, pollTask as pollTripoTask } from './services/photo-to-3d.js';
...
const componentLogger = createComponentLogger('KozaToolsController');
...
async function nanoBanana(req, res) {
  try {
    ...
    const { buffer, mimeType } = await generateNanoBanana({ prompt, referenceImages, stylePrompt });
...
async function sjinnCreate(req, res) {
  try {
    ...
    if (!tool_type || !input) {
      return res.status(400).json({ error: 'VALIDATION_ERROR', message: 'tool_type and input are required' });
    }
...
async function photoTo3dCreate(req, res) {
  try {
    ...
    componentLogger.error('Photo-to-3D create failed', { error: error.message });
    res.status(500).json({ error: 'INTERNAL_ERROR', message: error.message });
  }
}
...
export const kozaToolsController = {
  nanoBanana,
  ambientImage,
  ambientPrompt,
