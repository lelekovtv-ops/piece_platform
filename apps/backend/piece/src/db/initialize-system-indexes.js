...
export async function initializeSystemIndexes() {
  ...
  await authSessions.createIndex(
    { revokedAt: 1 },
    {
  ...
  await authAuditLog.createIndex(
    { createdAt: 1 },
    { expireAfterSeconds: 90 * 24 * 60 * 60 },
  );

  const deviceCodes = getGlobalSystemCollection("device_codes");
  await deviceCodes.createIndex({ deviceCode: 1 }, { unique: true });
  await deviceCodes.createIndex({ userCodeHash: 1, status: 1 });
  await deviceCodes.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 });

  const desktopTokens = getGlobalSystemCollection("desktop_tokens");
  await desktopTokens.createIndex({ tokenHash: 1 }, { unique: true });
  await desktopTokens.createIndex({ userId: 1, status: 1 });
  await desktopTokens.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 });

  const licenses = getGlobalSystemCollection("licenses");
  await licenses.createIndex({ userId: 1, productId: 1, status: 1 });
  await licenses.createIndex({ userId: 1, status: 1 });
